package com.example.andoid.notepad_kotlin.db

import android.content.ContentValues
import android.content.Context
import android.database.sqlite.SQLiteDatabase
import android.provider.BaseColumns
import com.example.andoid.notepadkotlin2.ListItem
...
class MyDbManager(context: Context) {
    ...
    fun insertToDb ( title : String, content : String, uri: String, time:String){
        val values = ContentValues().apply {
            ...
            put(MyDbNameClass.COLUMN_NAME_SUBTITLE,content)
       ...
       val dataList = ArrayList<ListItem>()
        ...
        val cursor = db?.query(MyDbNameClass.TABLE_NAME, null, selection, arrayOf("%$searchText%"),
            ...
            while (cursor?.moveToNext()!!) {
                ...
                val dataContent = cursor.getString(cursor.getColumnIndex(MyDbNameClass.COLUMN_NAME_SUBTITLE))
